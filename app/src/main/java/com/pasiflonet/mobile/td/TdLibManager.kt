...
object TdLibManager {
    ...
    private val _authState = MutableStateFlow("NOT_INITIALIZED")
    ...
    private fun handleAuthState(state: TdApi.AuthorizationState, apiId: Int, apiHash: String) {
        when (state) {
            is TdApi.AuthorizationStateWaitTdlibParameters -> {
                ...
                _authState.value = "WAIT_TDLIB_PARAMS"
            }
            is TdApi.AuthorizationStateWaitPhoneNumber -> _authState.value = "WAIT_PHONE"
            is TdApi.AuthorizationStateWaitCode -> _authState.value = "WAIT_CODE"
            is TdApi.AuthorizationStateWaitPassword -> _authState.value = "WAIT_PASSWORD"
            is TdApi.AuthorizationStateReady -> { isAuthorized = true; _authState.value = "READY" }
            is TdApi.AuthorizationStateClosed -> _authState.value = "CLOSED"
            else -> _authState.value = state.javaClass.simpleName
        }
    }

    fun sendPhone(phone: String) {
        client?.send(TdApi.SetAuthenticationPhoneNumber(phone, null)) { r ->
            if (r is TdApi.Error) Log.e("TdLib", "sendPhone error: ${r.message}")
        }
    }
    fun sendCode(code: String) {
        client?.send(TdApi.CheckAuthenticationCode(code)) { r ->
            if (r is TdApi.Error) Log.e("TdLib", "sendCode error: ${r.message}")
        }
    }
    fun sendPassword(password: String) {
        client?.send(TdApi.CheckAuthenticationPassword(password)) { r ->
            if (r is TdApi.Error) Log.e("TdLib", "sendPassword error: ${r.message}")
        }
    }
